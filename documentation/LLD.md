...
│   │   ├── schema.py           # Role-filtered schema fetcher (Semantic RBAC)
│   │   ├── generate.py         # SQL generation (LLM)
│   │   ├── validate.py         # SQL AST validation (sqlglot)
│   │   ├── execute.py          # SQL execution with ABAC enforcement
│   │   ├── masking.py          # DLP: PII scrubbing of SQL results [NEW]
...
│   │   ├── approval.py         # HITL approval pending routing node
│   │   └── general.py          # General web-search agent
│   │
...
│   │   ├── pii.py              # Presidio PIIScrubber (detect + tokenize PII)
│   │   ├── abac.py             # ABAC policy engine & PolicyDecisionPoint [NEW]
│   │   ├── schema_policy.py    # Role-filtered schema allowlist [NEW]
│   │   ├── security.py         # Agent RBAC identity & SecurityManager
│   │   ├── hitl.py             # HITL approval manager
...
**SQL Agent Graph Flow (Updated with Security Layers):**
```
input_guardrail → orchestrator → schema (role-filtered) → generate
 → validate (AST+HITL) → execute (ABAC) → masking (DLP) → evaluate → chart → format → output_guardrail
               ↘ approval_pending (HITL — admin approves via API) ↗
               ↘ retry (on validation/eval failure) ↗
```
...
#### 2.3.4 Validate Node (AST-based, sqlglot)
...
def validate_node(state: AgentState) -> Dict[str, Any]:
    ...
    Security improvements over old regex:
    - Root-node check: root must be exp.Select (blocks DROP, INSERT, UPDATE)
    - Multi-statement rejection: 'SELECT 1; DROP TABLE users' -> BLOCKED
    - Full tree traversal: subqueries, CTEs, JOINs all table-checked
    - HITL trigger: sensitive tables routed to approval_pending
    - Fallback: regex validator if sqlglot unavailable (with warning)
    """
```

#### 2.3.5 Execute Node (with ABAC)

```python
def execute_node(state: AgentState) -> Dict[str, Any]:
    """
    Execute SQL with Attribute-Based Access Control enforcement.

    Logic:
    1. Parse SQL AST to extract all referenced table names
    2. For each table: call pdp.evaluate(subject, resource, action, context)
       - subject = {role, user_id} from JWT-decoded state
       - resource = {table: <table_name>}
       - action = 'EXECUTE_SQL'
       - context = {requires_approval, approval_status}
    3. If any table DENIED -> return error, SQL never reaches DB
    4. Execute approved SQL against data_engine (SQLAlchemy)
    5. Return {'sql_result': [...], 'error': None}
    """
```

#### 2.3.5b Masking Node (DLP — NEW)

```python
def masking_node(state: AgentState) -> Dict[str, Any]:
    """
    Scrub PII from raw SQL result rows before the LLM formats them.

    Logic:
    1. Get sql_result list of dicts from state
    2. For each row, for each string column value:
        a. Run through PIIScrubber.scrub_text()
        b. PII -> replaced with vault token [PII_TYPE_xxxxxxxx]
    3. Return masked rows into state

    Token -> original mapping restored in main.py via deanonymize_text()
    before final JSON is sent to the frontend.
    """
```
