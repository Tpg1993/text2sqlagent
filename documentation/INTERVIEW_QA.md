...
A **traditional REST API** is purely **reactive and stateless**. It follows a fixed input→output contract: the client sends a request, the server performs a predefined operation, and immediately returns a deterministic response. The server has no memory, makes no autonomous decisions, and cannot "think" about how to answer.

The **agentic API** in this system is fundamentally different across several dimensions:

| Dimension | Traditional API | Agentic API (This System) |
|---|---|---|
| **Decision Making** | Fixed logic, no decisions | LLM-powered intent classification at runtime |
| **Execution Path** | Single, hardcoded code path | Dynamic multi-step graph that changes per query |
| **State** | Stateless per request | Stateful `AgentState` that flows through nodes |
| **Error Handling** | Return error code | Retry loop with LLM self-correction (up to 3 times) |
| **Tools** | Fixed function | Agent selects and uses tools (SQL execution, vector search) |
| **Response** | Deterministic | Generated by LLM, validated, and formatted |
| **Security** | Input validation middleware | Multi-layer: guardrails → RBAC → output validation |

In this system, a single `/chat` request triggers an **entire LangGraph workflow** — input guardrail → intent classifier → SQL/RAG/General pipeline → output guardrail → de-anonymization. The path taken depends on the semantics of the question, not the URL called.
